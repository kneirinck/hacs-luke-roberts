...
class LukeRobertsLuvoBleLight(LightEntity):
    ...
    async def _update_effect(self) -> None:
        ...
        _LOGGER.warning("Current scene id %s", current_scene_id)
        for scene_name, scene_id in self._effect_map.items():
            _LOGGER.warning("%s %d", scene_id, scene_id == current_scene_id)
